...
function drawStarbCourse(data,thisweek) {
  // draws table showing absent teachers
    ...
    s += '<tr id="starbrow"><th colspan="6" title="click to hide/show">'
            + 'Starbkurs denne uka <span class="info">(peker over for detaljer)</span></th></tr>';
    s += '<tr>';
    for (var i=0;i<6;i++) {
        if (header[i]) {
...
function drawAbsentees(data,thisweek) {
  // draws table showing absent teachers
    ...
    wholeweek += '</ul>';
    s += "<tr><th colspan=5>Fravær denne uka: <span class=\"info\">"
...
function getYearPlanThisWeek(thisweek) {
  // fetch weekly summary from yearplan
    ...
    var title = 'Denne Uka';
    for (var w=0;w<2;w++) {
      s += '<tr><th colspan=6><span style="font-size:0.7em;" >'+title+'</span></th></tr>';
      s += "<tr>";
