          ...
          client.query( "select * from quiz_useranswer where qid = $1 and userid = $2 and cid=$3 and instance=$4",[ qid,uid,cid,iid ],
            after(function(results) {
                  if (results && results.rows && results.rows[0]) {
                    ...
                    //console.log("Which to use -- ",myquest,qua);
...
exports.gettags = function(user,query,callback) {
  ...
  after(function(results) {
      ...
      // we now have direct tags - need to also gather tags on parent
      client.query( "select distinct q.teachid,t.tagname from quiz_tag t inner join quiz_qtag qt on (qt.tid=t.id) inner join quiz_question q on (q.parent = qt.qid) "
          + " where q.parent != 0 and q.teachid = $1 and q.subject in "+sublist+" and q.status != 9 order by t.tagname ", [uid],
      after(function(results) {
          if (results && results.rows && results.rows[0]) {
            for (var i=0,l=results.rows.length; i<l; i++) {
              var ta = results.rows[i];
  ...
  if (goodlist ) {
    ...
    after(function(stats) {
      ...
      client.query("select q.id,q.name,count(u.id) as cc from quiz_question q inner join quiz_useranswer u "
                   ...
                   + "u.userid in (" + studlist + ") and q.teachid=$2 group by q.id,q.name order by cc", [subject,teachid],
...
var setnewqlist = exports.setnewqlist = function(user,query,callback) {
  // update qlist to be equal to current qlist for all studs
  ...
  if (isteach) {
          if (db.memlist[group]) {
              ...
              after(function(allus) {
                  if (!(allus && allus.rows)) return;
                  var all_answers = {};
                  _.each(allus.rows,function(elm){ all_answers[elm.userid] = elm;  });  // indexed by userid
                  client.query("select * from quiz_question where id=$1", [container],
                  after(function(master) {
                    if (master && master.rows) {
                        var massa = master.rows[0];
                        if (massa && massa.qtext) {
                            var qopts =parseJSON(massa.qtext);
                            // we now have original qlist
                            var newqlist = qopts.qlistorder.join(',');
                            //console.log("SET NEW QLIST: ",newqlist);
                            for (var i=0, l = db.memlist[group].length; i<l; i++) (function(enr) {
                                var usa = all_answers[enr];
                                if (usa) {
                                  var pa =parseJSON(usa.param);
                                  //console.log(enr,pa.qlistorder);
                                  pa.qlistorder = newqlist;
                                  usa.param = JSON.stringify(pa);
                                  //console.log(enr);
                                  client.query("update quiz_useranswer set param = $1 where id=$2", [usa.param,usa.id]);
                                }
                            }) (db.memlist[group][i]);
                        }
                    }
